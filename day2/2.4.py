...
def get_best_move(board, depth):
    ...
    for move in board.available_moves():
        board.make_move(move, 'O')
        eval = alphabeta(board, depth, alpha, beta, False)
        board.board[move] = ' '  # undo move
        if eval > best_eval:
            best_eval = eval
            best_move = move
...
def play_game():
    # Function to play the Tic Tac Toe game against the computer using Alpha-Beta Pruning algorithm
    game = TicTacToe()
    ...
    while not game.is_board_full():
        ...
        if current_player == player1:
            ...
            move = get_best_move(game, 5)  # Set depth to 5 for deeper search
            game.make_move(move, current_player)
